...
import (
	...
	"github.com/alauda/redis-operator/internal/controller/redis/redisbackup"
...
type RedisBackupReconciler struct {
	...
	handler   *redisbackup.RedisBackupHandler

	Logger logr.Logger
	Scheme *runtime.Scheme
}

const RedisBackupFinalizer = "redisbackups.redis.middleware.alauda.io/finalizer"

//+kubebuilder:rbac:groups=redis.middleware.alauda.io,resources=redisbackups,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=redis.middleware.alauda.io,resources=redisbackups/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=redis.middleware.alauda.io,resources=redisbackups/finalizers,verbs=update
...
func (r *RedisBackupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if isMarkedToBeDeleted {
		...
		if err := r.finalizeRedisBackup(ctx, logger, &obj); err != nil {
			if obj.Status.Message != err.Error() {
				...
				if _err := r.k8sClient.UpdateRedisBackupStatus(ctx, &obj); _err != nil {
	...
	if err := obj.Validate(); err != nil {
		...
		obj.Status.Message = err.Error()
		if err := r.k8sClient.UpdateRedisBackupStatus(ctx, &obj); err != nil {
...
func (r *RedisBackupReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&middlewarev1.RedisBackup{}).
		Complete(r)
}

// SetupHandler
func (r *RedisBackupReconciler) SetupHandler(mgr ctrl.Manager) {
	r.k8sClient = clientset.New(mgr.GetClient(), r.Logger)

	client := service.NewRedisBackupKubeClient(r.k8sClient, r.Logger)
	r.handler = redisbackup.NewRedisBackupHandler(r.k8sClient, client, r.Logger)
}

func (r *RedisBackupReconciler) finalizeRedisBackup(ctx context.Context, reqLogger logr.Logger, backup *middlewarev1.RedisBackup) error {
	if backup.Spec.Target.S3Option.S3Secret != "" {
		if err := r.handler.DeleteS3(ctx, backup); err != nil {
			r.Logger.Info("DeleteS3", "Err", err.Error())
			return err
		}
	}
	return nil
