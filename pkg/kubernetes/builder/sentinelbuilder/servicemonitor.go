...
func NewServiceMonitorForCR(rf *v1.RedisFailover, sentnelLabels map[string]string) *smv1.ServiceMonitor {
	...
	if rf != nil {
		if rf.Spec.ServiceMonitor.Interval != "" {
			interval = rf.Spec.ServiceMonitor.Interval
		}
		if rf.Spec.ServiceMonitor.ScrapeTimeout != "" {
			scrapeTimeout = rf.Spec.ServiceMonitor.ScrapeTimeout
		}
		if rf.Spec.ServiceMonitor.CustomMetricRelabelings {
			configs = rf.Spec.ServiceMonitor.MetricRelabelConfigs
		}
	}
	...
	sm := &smv1.ServiceMonitor{
		...
		Spec: smv1.ServiceMonitorSpec{
			...
			TargetLabels: []string{util.LabelRedisArch},
		},
	}
