...
func NewRedisClusterShard(ctx context.Context, client clientset.ClientSet, cluster types.RedisClusterInstance, sts *appv1.StatefulSet, logger logr.Logger) (types.RedisClusterShard, error) {
	...
	users := cluster.Users()
	var err error
	if shard.nodes, err = models.LoadRedisNodes(ctx, client, sts, users.GetOpUser(), logger); err != nil {
...
func (s *RedisClusterShard) Slots() *slot.Slots {
	...
	for _, node := range s.nodes {
		if node.IsJoined() && node.Role() == redis.RedisRoleMaster &&
...
func (s *RedisClusterShard) Refresh(ctx context.Context) error {
	...
	var err error
	if s.nodes, err = models.LoadRedisNodes(ctx, s.client, &s.StatefulSet, s.cluster.Users().GetOpUser(), logger); err != nil {
